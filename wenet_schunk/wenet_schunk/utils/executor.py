...
class Executor:
            ...
            for batch_idx, batch in enumerate(data_loader):
                ...
                with context():
                    ...
                    with torch.cuda.amp.autocast(scaler is not None):
                        ...
                        loss_att = loss_att /accum_grad
                        loss_ctc = loss_ctc / accum_grad
                    if use_amp:
                ...
                if batch_idx % log_interval == 0:
                    ...
                    if loss_att is not None:
                        log_str += 'loss_att {:.6f} '.format(loss_att.item()*accum_grad)
                    if loss_ctc is not None:
                        log_str += 'loss_ctc {:.6f} '.format(loss_ctc.item()*accum_grad)
                    log_str += 'lr {:.8f} rank {}'.format(lr, rank)
                    logging.debug(log_str)

                total_loss += loss.item() * num_utts
                total_attn_loss += loss_att.item() * num_utts
                total_ctc_loss += loss_ctc.item() * num_utts

        return total_loss, total_attn_loss, total_ctc_loss, num_seen_utts
    ...
    def cv(self, model, data_loader, device, args):
        ...
        log_str = 'CV Batch {} total_avg_loss {:.6f} total_avg_ctc_loss {:.6f} total_avg_attn_loss {:.6f} '.format(epoch, 
                total_loss/num_seen_utts, total_ctc_loss/num_seen_utts, total_attn_loss/num_seen_utts )
        logging.debug(log_str)
