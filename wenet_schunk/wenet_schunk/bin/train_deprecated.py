...
the future, please move to the new IO !!!
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        total_loss, total_attn_loss, total_ctc_loss, num_seen_utts = executor.cv(model, cv_data_loader, device,
                                                configs)
        if args.world_size > 1:
            ...
            total_attn_loss = torch.Tensor([total_attn_loss]).to(device)
            dist.all_reduce(total_attn_loss)
            cv_attn_loss = total_attn_loss[0] / num_seen_utts[0]
            cv_attn_loss = cv_attn_loss.item()

            total_ctc_loss = torch.Tensor([total_ctc_loss]).to(device)
            dist.all_reduce(total_ctc_loss)
            cv_ctc_loss = total_ctc_loss[0] / num_seen_utts[0]
            cv_ctc_loss = cv_ctc_loss.item()
        else:
            cv_loss = total_loss / num_seen_utts
            cv_attn_loss = total_attn_loss / num_seen_utts
            cv_ctc_loss = total_ctc_loss / num_seen_utts

        logging.info('Epoch {} CV info cv_loss {}, cv_attn_loss {}, cv_ctc_loss {}'.format(epoch, cv_loss, cv_attn_loss, cv_ctc_loss))

        total_tr_loss, total_tr_attn_loss, total_tr_ctc_loss, num_seen_utts = executor.train(model, optimizer, 
                scheduler, train_data_loader, device, writer, configs, scaler)
        ...
        if args.world_size > 1:
            ...
            total_tr_loss = torch.Tensor([total_tr_loss]).to(device)
            dist.all_reduce(total_tr_loss)
            tr_loss = total_tr_loss[0] / num_seen_utts[0]
            tr_loss = tr_loss.item()

            total_tr_attn_loss = torch.Tensor([total_tr_attn_loss]).to(device)
            dist.all_reduce(total_tr_attn_loss)
            tr_attn_loss = total_tr_attn_loss[0] / num_seen_utts[0]
            tr_attn_loss = tr_attn_loss.item()

            total_tr_ctc_loss = torch.Tensor([total_tr_ctc_loss]).to(device)
            dist.all_reduce(total_tr_ctc_loss)
            tr_ctc_loss = total_tr_ctc_loss[0] / num_seen_utts[0]
            tr_ctc_loss = tr_ctc_loss.item()
        else:
            tr_loss = total_tr_loss / num_seen_utts
            tr_attn_loss = total_tr_attn_loss / num_seen_utts
            tr_ctc_loss = total_tr_ctc_loss / num_seen_utts

        logging.info('Epoch {} CV info tr_loss {}, tr_attn_loss {}, tr_ctc_loss {}'.format(epoch, tr_loss, tr_attn_loss, tr_ctc_loss))
