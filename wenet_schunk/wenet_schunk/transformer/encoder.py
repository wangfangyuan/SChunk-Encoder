...
from wenet.transformer.attention import ChunkAttention
...
from wenet.transformer.encoder_layer import SChunkTransformerEncoderLayer
from wenet.transformer.encoder_layer import SChunkConformerEncoderLayer
...
class BaseEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        #CT= xs.size(1)
        #xs = F.pad( xs, pad=(0, 0, 0, 384-CT ), value=0.0)
        #masks = F.pad(masks.int(), pad=(0,384-CT ), value=0.0) > 0

        if True:
            T = xs.size(1)
            if T%self.static_chunk_size == 0:
                self.input_resolution = T
            else:
                self.input_resolution = int(T/self.static_chunk_size+1)*self.static_chunk_size

            pad_len = self.input_resolution - T

            if pad_len >= 0:
                xs = F.pad( xs, pad=(0, 0, 0, pad_len), value=0.0)
                masks = F.pad(masks.int(), pad=(0,pad_len), value=0.0) > 0
            else:
                xs = xs[:,:self.input_resolution,:]
                masks = masks[:,:,:self.input_resolution]
                print( "larger than :", self.input_resolution, "T=", T )
    ...
    def forward_chunk(
        ...
        """
        #print( "encoder.py 234: xs.shape = ", xs.shape )
        assert xs.size(0) == 1
        ...
        #print( "encoder.py 259: xs.shape = ", xs.shape )
        #2022.02.11 wfy, added
        masks_pad = masks
        #print( "encoder.py 263: masks_pad.shape = ", masks_pad.shape )
    ...
    def forward_chunk_by_chunk(
        ...
        #print( "encoder.py 338: xs.shape = ", xs.shape )
        #print( "encoder.py 338: decoding_window = ", decoding_window )
        # Feed forward overlap input step by step
        for cur in range(0, num_frames - context + 1, stride):
            end = min(cur + decoding_window, num_frames)
            #chunk_xs = xs[:, cur:end, :]
            #wfy,20220513,for schunk
            chunk_xs = xs[:, :end, :]
...
class SChunkTransformerEncoder(BaseEncoder):
    """Transformer encoder module."""
    def __init__(
        self,
        input_size: int,
        ...
        """ Construct SChunkTransformerEncoder
        ...
        # build blocks
        self.encoders = torch.nn.ModuleList([
            SChunkTransformerEncoderLayer(
                                size=output_size, 
                                self_attn = ChunkAttention(attention_heads, output_size,attention_dropout_rate, static_chunk_size),
                                feed_forward = PositionwiseFeedForward(output_size, linear_units, positional_dropout_rate),
                                dropout_rate = dropout_rate,
                                ...
                                #shift_size=0 
                                shift_size=0 if (i % 2 == 0) else static_chunk_size // 2
                                )
            for i in range(num_blocks)])

        self.input_resolution = input_resolution

class SChunkConformerEncoder(BaseEncoder):
    """Conformer encoder module."""
    def __init__(
        self,
        input_size: int,
        ...
        self.encoders = torch.nn.ModuleList([
            SChunkConformerEncoderLayer(
                output_size,
                ChunkAttention(attention_heads, output_size,attention_dropout_rate, static_chunk_size),
                positionwise_layer(*positionwise_layer_args),
                positionwise_layer(
                    *positionwise_layer_args) if macaron_style else None,
                convolution_layer(
                ...
                input_resolution,
                static_chunk_size,
                0 if (i % 2 == 0) else static_chunk_size // 2
            ) for i in range(num_blocks)
        ])
